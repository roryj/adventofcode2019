...
fun part1(wire1Points: Map<Point, Int>, wire2Points: Map<Point, Int>) {
    ...
    println("part1 complete")
}

fun part2(wire1Points: Map<Point, Int>, wire2Points: Map<Point, Int>) {
...
//        println("Looking for intersect on $point")
    ...
    println("min step count: $combinedStepCount")
    println("part2 complete")
}

fun findClostStepIntersectTime() {
...
/**
 * Gets all the points that a wire path crosses
 *
 * @return returns a map where the key is the wire point, and the value is the step number that led to that value
 */
fun getAllWireTouchPoints(commands: List<String>): Map<Point, Int> {
    ...
    commands.forEach {
        ...
        for (step in 1..movement.steps) {
            ...
            println("visiting point ${currentPoint.x},${currentPoint.y} @ step #$currentStep")

            // only add this again if we have not seen it before. The first time we touch a point with a wire is the
            // important one
            if (!visitedPoints.containsKey(currentPoint)) {
                visitedPoints.put(currentPoint, currentStep)
            }
            currentStep++
        }
    }
